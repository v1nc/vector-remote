...
def get_cert(serial=None):
    ...
    sys.stdout.flush()
    r = requests.get('https://session-certs.token.global.anki-services.com/vic/{}'.format(serial),verify=False)
...
def get_session_token(api, username,password):
...
def generate_config(input_name,input_ip,input_serial,input_email,input_password):
    api = Api()
    name, ip = get_name_and_ip(input_name,input_ip)
    cert, serial = get_cert(input_serial)
    ...
    token = get_session_token(api, input_email,input_password)
